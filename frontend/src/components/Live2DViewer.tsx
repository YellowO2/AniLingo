    ...
    (async () => {
      try {
        ...
        const startIdleMotion = () => {
          const motions = ["Idle", "Breath"]; // Reduced to softer motions
          const randomMotion =
            motions[Math.floor(Math.random() * motions.length)];
          model.motion(randomMotion);

          // Schedule next motion with longer delay
          setTimeout(startIdleMotion, 10000 + Math.random() * 10000); // 10-20 seconds
        };
